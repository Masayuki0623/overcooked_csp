...
画面の下に矢印ボタンと「AIに指示」「閉じる」ボタンがあり、キーボードの無いスマホでも同じ操作ができます（指示パネルのカードはタップで選べます）。画面が狭いと、ゲーム画面は幅に合わせて縮み、指示パネルはゲーム画面の下に出ます。ボタンは押した瞬間に1回だけ入力され、押しっぱなしでは連続しません（キーボード側もキーリピートを無視しているのとそろえています）。
