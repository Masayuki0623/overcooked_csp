...
- **「使う」を押しっぱなしにしても、置く・取るは1回だけです。** 切る・混ぜるは押している間ずっと続きます。ただし**まな板に置いたときだけは例外**で、そのまま「置く→切る→取る」まで1回の長押しでできます。取り上げた時点でその長押しは終わりで、もう一度押すと置けます（押しっぱなしのまま、拾ってすぐ置き直してしまうのを防ぐため）。
- **キャラの絵は向きに合わせて変わります**（前・後ろ・横の3種類。横は左右で反転）。絵は `python tools/gen_agent_sprites.py` で作り直せます。向いている先のマスには青い枠も出ます。
