...
### 複数人が同時に遊ぶ (`server_multi.py`)

`server.py` 1つでは同時に1人しか遊べません（pygame の画面はプロセスに1つしか持てないため）。`server_multi.py` は人数ぶんの `server.py` を裏で立て、**URL 1つのまま**、来た人を空いている席へ回します。参加者に配る URL も cloudflared / Tailscale の設定も今までどおりで構いません。

```bash
python server_multi.py                  # 4人ぶん（既定）
python server_multi.py --players 2
python server_multi.py --map exp_ring   # server.py の引数はそのまま渡ります
```

- 席は端末ごとの cookie で覚えます。遊んでいる途中で読み込み直しても同じ席に戻ります。全部ふさがっていれば、これまでどおり「別の人がプレイ中」の画面が出ます。
- 席の状況は `http://localhost:8000/__seats` で見られます。
- 記録のファイル名には席の番号が入ります（`web0-...rep` / `...-i0-game0.json`）。同じ秒に複数人が終わっても混ざりません。`assignments.json` と CSV はプロセスをまたぐ鍵で守っています。

**人数の上限は 4 です**（`MAX_PLAYERS`）。このPC（i7-11700 8コア / 16GB）で測った値は次のとおりです。

| | 1人 | 2人 | 4人 |
| --- | --- | --- | --- |
| AI の判断（中央値） | 46ms | 49ms | 56ms |
| 同（99%） | 135ms | 207ms | 178ms |
| 1コマ（200ms）に間に合わなかった割合 | 0.0% | 1.3% | 0.5% |

4人を実際に同時につないだときも、ゲームの1周は 201〜203ms（目標 200ms）、描画は 5.0fps で目標どおりでした。1人ぶんの消費は CPU 0.17コア・メモリ 264MB です。
...
| 選ぶもの | 選択肢 |
| --- | --- |
| 地図 | **仕切り**（`exp_partition`、左右が行き来できない）/ **リング**（`exp_ring`、真ん中の島のまわりを回れる） |
| レシピ | **野菜のみ**（`experiment1`、サラダ2品＋スープ1品、27通り）/ **野菜＋フルーツ**（`experiment2`、サラダ＋スープ＋ジュース、18通り） |
| 注文の組み合わせ | そのレシピの組み合わせから1つ、または「おまかせ」（毎回ランダム） |
| AIへの指示 | **指示あり（最初に1回）** / **指示なし** |
| 指示の効き方（skip_budget） | **0 / 2 / 4**（指示ありのときだけ選べます）|

skip_budget は「指示した作業に取りかかる前に、AI が他の作業を何個まで挟んでよいか」の上限です。0 ならすぐ取りかかり、4 なら他を4つまで先にやります。**上限であって目標ではない**ので、もともとの計画がその範囲に収まっていれば、値を上げても動きは変わりません。指示した作業の前提工程（同じ注文で先に必要な工程）と、相手の作業は数えません。指示された作業は必ず AI の担当になります。

指示ありのときは、**3・2・1 のカウントダウンの前**に指示の選択が出ます。画面を覆わず、移動ボタンと同じ場所（盤面の下）に、同じ高さで出るので、**盤面の位置は動かず**、見ながら選べます。選んでいる間は移動・使うボタンを隠します。時間制限はありません（選ぶまでゲームの時間は止まっています）。選んで決めるとカウントダウンが始まり、ゲームが動き出します。上には「どちらがあなたで、どちらが AI か」をキャラの絵で示します。
...
- 条件は **地図2種 × AI が指示を後回しにできる量（skip_budget 0 / 2 / 4）= 6通り**。1人が6セッション全部を遊び、**順番だけ人ごとにランダム**です。並びは `results/assignments.json` に残るので、途中でサーバーを止めても続きから遊べます。
- **注文の構成は地図ごとに決まります。** 仕切りは `experiment2`（サラダ＋スープ＋ジュース）、リングは `experiment1`（サラダ2品＋スープ）です。リングはフルーツを使わないので、地図もフルーツ・ミキサー・コップを外した版（`exp_ring_veg`）に自動で切り替わります。どちらも、指示の良し悪しが判別できる構成だけをランダムに選びます。指示は開始直後に1回だけ（見送り不可）です。
- 途中で接続が切れた回は記録に残しますが、セッション数には数えません（同じ条件をやり直します）。

セッションが終わるとアンケートが出ます（選択14項目・1分程度＋自由記述）。パートAは協調感（CCR尺度 Coordination 因子、6項目・5段階）、パートBはAIパートナーへの信頼感（MDMT v1 Capacity Trust、8項目・0〜7の8段階＋「あてはまらない」）です。選択14項目は必須で、「あてはまらない」は欠損として平均から除きます（0点ではありません）。パートCは自由記述3欄（噛み合ったところ／気になったところ／その他）で、**任意**です。空のままでも送信できます。

| ファイル | 内容 |
| --- | --- |
| `results/survey.csv` | 1行 = 1セッションの回答。`coord_1`〜`coord_6` と平均 `coord_mean`、`trust_1`〜`trust_8` と平均 `trust_mean`、「あてはまらない」の数 `trust_dnf_count`、自由記述 `free_good` / `free_bad` / `free_other`、条件（`map` / `skip_budget` / `case`）と結果（`served` / `makespan_s`） |
| `results/web_sessions.csv` | そのセッションの客観データ。提供数・所要時間・打ち切りに加えて、**指示の内容**（`instruction` / `instruction_verb` / `instruction_obj` / `quality`）と**効き方**（`wait_seconds` = 指示してから着手までの秒数、`exec_rank` = 何番目にやったか、`natural_rank` = 指示しなければ何番目だったか、`rank_gain` = 繰り上がった数、`tasks_before` = 間に挟んだ数）|
