...
class CSPAgent:
    ...
    #   〜待機中         : 相手が材料を持ってくる・鍋を空けるのを待っている
    # どちらも待っていても自分では何も変えられない。その作業を一旦諦めて
    # 別の割り当てへ回った方が、結果として早く片づく(諦めた作業は 30 秒で
    # 候補に戻る)。
    #
    # 「待ち」で終わる理由(調理完了待ち・受け渡し待ち)は含めない。そちらは
    # 放っておけば進む、正当な待ち時間。
    _STUCK_REASON_MARKS = ('が見つかりません', '待機中')
...
#
    ...
    def solve_csp_scheduling(self, env, orders):
        ...
        # 相手(人間)の出発地点も、実際にいる場所をそのまま使う。
        #
        # 以前は、両者が同じ区画にいるとき相手の出発地点を自分と同じ場所に
        # 置き換えていた。人間が歩き回るたびに距離行列が変わり、「どちらが
        # どのタスクを担当するか」の決定が入れ替わって、食材を置く→拾うを
        # 繰り返す揺れが出たためだった。仕切りのある地図では、相手側にしか
        # ないタスクが到達不能になるので置き換えていなかった。
        #
        # ただしこれは、リングの地図では「人間はAIの隣にいる」という事実と
        # 違う前提で費用を見積もることになる。人間が地図の反対側にいても
        # 同じ見積もりになるため、計画が実態からずれる。揺れは担当の入れ替え
        # に罰を与える仕組み(switch_penalty)で押さえる方針にして、位置は
        # 正確に使う。
